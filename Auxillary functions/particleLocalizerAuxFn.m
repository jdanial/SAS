...
cameraPixelSize = app.param.detection.cameraPixelSize;
...
%% extracting image
image = ((double(app.data.file(fileId).image) - cameraOffset) ./ cameraEMGain) ./ (cameraQE / 100);
...
for particleId = 1 : numParticles
    if strcmp(app.data.file(fileId).particle(particleId).state,'accepted')
        ...
        centroids(particleId,:) = [app.data.file(fileId).particle(particleId).centroid.y ...
...
for particleId = 1 : size(fitData,2)
    if states(particleId) == 0
        ...
        % updating sigma
        app.data.file(fileId).particle(indices(particleId)).sigma = parameters(4,particleId) * cameraPixelSize; 
    else
        app.data.file(fileId).particle(indices(particleId)).state = 'rejected';
    end
end
end